...
|--------------------|--------------------|
| Check Apache syntax | Use `httpd -t` to check **Apache config syntax** before reloading |
| Reload Apache service | Always `systemctl reload httpd` *(or `systemctl restart httpd`)* after editing config files |
| Restore SELinux contexts | Use `restorecon -Rv <path>` to restore SELinux contexts if access fails |
| Fix SELinux user-dir errors | `setsebool -P httpd_read_user_content 1` + `setsebool -P httpd_enable_homedirs 1` fix most Apache/SELinux user-dir errors |
| Verify HTTP redirects | Use `curl -I <url>` to verify redirects (**HTTP 301** expected for rewrite rules) |
